    ...
    <>
      <nav className="fixed top-4 left-4 right-4 z-50 mx-auto max-w-7xl">
        <motion.div
          ...
          className="bg-white/40 backdrop-blur-2xl rounded-2xl shadow-2xl border border-white/10 px-6 py-4"
        >
          <div className="flex items-center justify-between">
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-orange-400 font-medium hidden md:block">Premium Fireworks</p>
              </div>
            </motion.div>
            ...
            <div className="hidden lg:flex items-center gap-1">
              {navItems.map((item, index) => (
                <motion.button
                  ...
                  className={`relative px-4 py-2 font-medium transition-all duration-300 rounded-xl group h-10 flex items-center justify-center ${activeTab === item.path
                    ? "text-orange-400 bg-white/10"
                    : "text-black hover:text-orange-400 hover:bg-white/5"
                    }`}
                >
                  {item.name}
                  {activeTab === item.path && (
            ...
            <motion.button
              ...
              className="flex items-center gap-2 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-semibold px-4 py-2 md:px-6 md:py-2.5 rounded-xl transition-all duration-300 shadow-lg shadow-orange-500/30 hover:shadow-xl text-sm md:text-base"
            >
      ...
      >
        <div className="bg-black/40 backdrop-blur-2xl shadow-lg rounded-2xl border border-white/10 p-2 flex justify-around items-center">
          {navItems.map((item) => {
              ...
              <div key={item.name} className="relative flex-1">
                {isActive && (
                  <motion.div
                    ...
                    className="absolute inset-0 bg-gradient-to-br from-orange-400 to-orange-600 rounded-xl"
                ...
                >
                  <Icon className={`w-6 h-6 ${isActive ? "text-white" : "text-gray-500"} z-10`} />
                </button>
              </div>
            )
          })}
