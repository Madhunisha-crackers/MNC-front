  ...
  useEffect(() => {
    const initializeData = async () => {
      ...
      try {
        ...
        if (!statesRes.ok || !productsRes.ok || !promocodesRes.ok) {
        ...
        // Fix: Access productsData.data and ensure it's an array
        const productsArray = Array.isArray(productsData.data) ? productsData.data : [];
        setProducts(productsArray.filter((p) => p.status === "on"));
        setPromocodes(Array.isArray(promocodesData) ? promocodesData : []);
      } catch (err) {
        console.error("Error loading initial data:", err, { productsData });
      } finally {
        setIsLoading(false); // Reduced timeout for better UX
      }
    };
    initializeData();
  }, []);
