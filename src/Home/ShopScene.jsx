...
export default function ShopScene({ onClose }) {
  ...
  return(
    <div style={{position:'fixed',inset:0,overflow:'hidden',userSelect:'none',touchAction:'none'}}>
      ...
      <div style={{position:'fixed',top:0,left:0,right:0,zIndex:40,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'8px 14px',background:'rgba(255,255,255,0.85)',borderBottom:'1px solid #ffd8a8',backdropFilter:'blur(12px)'}}>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          {onClose&&<button onClick={onClose} style={{...M,background:'#ff5500',color:'#fff',border:'none',padding:'5px 12px',borderRadius:7,cursor:'pointer',fontSize:'0.68rem'}}>← EXIT</button>}
          <span style={{...M,color:'#ff5500',fontSize:'0.72rem',letterSpacing:'0.1em'}}>🧨 MADHU NISHA CRACKERS</span>
          <span style={{...MS,color:'rgba(0,0,0,0.5)',fontSize:'0.62rem'}}>{products.length} products</span>
        </div>
        <button onClick={()=>setShowCheckout(true)} style={{...M,background:cartCount>0?'#ff5500':'#f3f4f6',color:cartCount>0?'#fff':'#374151',border:'1px solid #ffd8a8',padding:'6px 14px',borderRadius:20,cursor:'pointer',fontSize:'0.7rem',letterSpacing:'0.06em'}}>
          🛒 {cartCount} · ₹{fmt(cartRaw)}
        </button>
      </div>
      {nearBilling&&!modalOpen&&(
        <HUD>
          <div style={{background:'rgba(240,253,244,0.95)',backdropFilter:'blur(20px)',border:'2px solid #22c55e',borderRadius:18,padding:'20px 24px',boxShadow:'0 0 50px rgba(0,200,80,0.1)',textAlign:'center'}}>
            <div style={{fontSize:'1.8rem',marginBottom:4}}>💳</div>
            <div style={{...M,color:'#16a34a',fontSize:'1.05rem',letterSpacing:'0.12em',marginBottom:3}}>BILLING COUNTER</div>
            <div style={{...MS,color:'#374151',fontSize:'0.78rem',marginBottom:2}}>{cartCount} item{cartCount!==1?'s':''} · ₹{fmt(cartRaw)}</div>
            {!isMobile&&<div style={{...MS,color:'#4b5563',fontSize:'0.6rem',marginBottom:14}}>Press <kbd style={{background:'#e2e8f0',border:'1px solid #cbd5e1',padding:'1px 6px',borderRadius:4,color:'#1e293b',fontFamily:'monospace'}}>ENTER</kbd> or tap below</div>}
      ...
      {aimedProduct&&!nearBilling&&!modalOpen&&(
        <div style={{position:'fixed',bottom:isMobile?160:60,left:'50%',transform:'translateX(-50%)',zIndex:35,pointerEvents:'all',width:'min(380px,90vw)'}}>
          <div style={{background:'rgba(255,255,255,0.95)',backdropFilter:'blur(18px)',border:'1px solid #ffd8a8',borderRadius:16,overflow:'hidden',boxShadow:'0 8px 40px rgba(0,0,0,0.15)'}}>
            <div style={{display:'flex',gap:12,alignItems:'center',padding:'10px 14px 8px'}}>
              {(()=>{ const url=(aimedProduct?.images||[]).find(i=>i&&!i.includes('/video/')&&!i.toLowerCase().endsWith('.gif')); return url?<img src={url} alt="" style={{width:56,height:56,borderRadius:9,objectFit:'cover',background:'#f9fafb',flexShrink:0,border:'1px solid #ffd8a8'}} onError={e=>e.target.style.display='none'}/>:null })()}
              <div style={{flex:1,minWidth:0,textAlign:'left'}}>
                <div style={{...MS,color:'#000',fontSize:'0.82rem',fontWeight:'bold',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{aimedProduct?.productname||''}</div>
                <div style={{display:'flex',alignItems:'baseline',gap:6,marginTop:2,flexWrap:'wrap'}}>
                  <span style={{...M,color:'#ff5500',fontSize:'0.95rem'}}>₹{fmt((aimedProduct?.price||0)*(1-(aimedProduct?.discount||0)/100))}</span>
                  {(aimedProduct?.discount||0)>0&&<><span style={{color:'rgba(0,0,0,0.4)',textDecoration:'line-through',fontSize:'0.66rem'}}>₹{fmt(aimedProduct?.price||0)}</span><span style={{background:'#ff5500',color:'#fff',fontSize:'0.56rem',padding:'1px 5px',borderRadius:10,fontWeight:'bold'}}>{Math.round(aimedProduct?.discount||0)}% OFF</span></>}
                </div>
                <div style={{...MS,fontSize:'0.58rem',color:'rgba(0,0,0,0.5)',marginTop:1}}>/{aimedProduct?.per} · {aimedProduct?.serial_number}</div>
              </div>
            </div>
            <div style={{display:'flex',borderTop:'1px solid #ffe8cc'}}>
              <button onClick={()=>{ setViewProduct(aimedProduct); setViewImgIdx(0) }}
                style={{...M,flex:1,background:'rgba(59,130,246,0.1)',color:'#2563eb',border:'none',padding:'14px 8px',cursor:'pointer',fontSize:'0.82rem',letterSpacing:'0.04em'}}>
                👁 VIEW DETAILS
              </button>
            </div>
          </div>
      ...
      {!isMobile&&!nearBilling&&!aimedProduct&&!modalOpen&&(
        <div style={{position:'fixed',bottom:10,left:'50%',transform:'translateX(-50%)',zIndex:20,pointerEvents:'none'}}>
          <div style={{...MS,background:'rgba(255,255,255,0.85)',color:'#374151',padding:'4px 18px',borderRadius:18,fontSize:'0.6rem',border:'1px solid #ffd8a8',whiteSpace:'nowrap'}}>
      ...
      {isMobile&&!joyVis.active&&!aimedProduct&&!nearBilling&&!modalOpen&&(
        <div style={{position:'fixed',bottom:16,left:'50%',transform:'translateX(-50%)',zIndex:20,pointerEvents:'none'}}>
          <div style={{...MS,background:'rgba(255,255,255,0.85)',color:'#374151',padding:'4px 14px',borderRadius:16,fontSize:'0.6rem',border:'1px solid #ffd8a8',whiteSpace:'nowrap'}}>
            Drag screen to look · Left = move · Tap product to add
          </div>
        </div>
      )}
      {cartCount>0&&(
        <div style={{position:'fixed',top:50,right:0,zIndex:35,width:210,maxHeight:'50vh',overflowY:'auto',background:'rgba(255,255,255,0.9)',backdropFilter:'blur(16px)',borderLeft:'1px solid #ffd8a8',padding:'10px'}}>
          <div style={{...M,color:'#ff5500',fontSize:'0.68rem',letterSpacing:'0.12em',marginBottom:8,borderBottom:'1px solid #ffe8cc',paddingBottom:5}}>🛒 CART ({cartCount})</div>
          {Object.values(cart).map(({product:p,qty})=>(
            <div key={p.serial_number} style={{display:'flex',alignItems:'center',gap:5,marginBottom:6,background:'rgba(255,85,0,0.04)',borderRadius:7,padding:'4px 6px',border:'1px solid #ffe8cc'}}>
              <div style={{flex:1,minWidth:0}}>
                <div style={{...MS,color:'#1f2937',fontSize:'0.62rem',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{p.productname}</div>
                <div style={{...M,color:'#ff5500',fontSize:'0.62rem'}}>₹{fmt(p.price*(1-(p.discount||0)/100))} ×{qty}</div>
              </div>
              <button onClick={()=>removeFromCart(p.serial_number)} style={{background:'rgba(255,50,50,0.1)',color:'#dc2626',border:'none',borderRadius:4,width:20,height:20,cursor:'pointer',fontSize:'0.9rem',lineHeight:'18px',textAlign:'center'}}>−</button>
            </div>
          ))}
          <div style={{borderTop:'1px solid #ffe8cc',marginTop:5,paddingTop:5,textAlign:'right'}}><span style={{...M,color:'#ff5500',fontSize:'0.72rem'}}>₹{fmt(cartRaw)}</span></div>
        </div>
      )}
